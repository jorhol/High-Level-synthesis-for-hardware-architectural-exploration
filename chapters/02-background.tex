...
The \gls{sap-1} architecture has 6 states, T1-T6, where T1-T3 is the instruction cycle and T4-T6 is the execution cycle. Some instructions does not all 6 states to complete, but the architecture has a fixed length of 6 cycles per instruction. This means that some instructions perform "no  operation" (NOPs) in the cycles where no work needs to be done.